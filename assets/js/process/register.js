$(document).ready(function() {
	...
	$('form').submit(function(event) {
		...
		$.ajax({
			...
			url 		: 'http://amins-macbook-pro.local:5757/html-planium/main-planium/register_process.php', // the url where we want to POST
			...
			.done(function(data) {
				...
				if ( ! data.success) {
					...
					if (data.errors.lName) {
						$('#lName_group').addClass('has-error'); // add the error class to show red input
						$('#lName_group').append('<div class="help-block">' + data.errors.lName + '</div>'); // add the actual error message under our input
					}

					if (data.errors.fName) {
						$('#fName_group').addClass('has-error'); // add the error class to show red input
						$('#fName_group').append('<div class="help-block">' + data.errors.fName + '</div>'); // add the actual error message under our input
					}

					if (data.errors.mobile) {
						$('#mobile_group').addClass('has-error'); // add the error class to show red input
						$('#mobile_group').append('<div class="help-block">' + data.errors.mobile + '</div>'); // add the actual error message under our input
					}
					...
					// ALL GOOD!
					window.location.href = "http://amins-macbook-pro.local:5757/html-planium/main-planium/";
